...
public class PotElm extends CircuitElm implements Command, MouseWheelHandler {
    ...
    public void setPoints() {
        ...
        int myLen = 0;
        if (abs(dx) > abs(dy)) {
            myLen = 2 * sim.gridSize * Integer.signum(dx) * ((Math.abs(dx) + 2 * sim.gridSize - 1) / (2 * sim.gridSize));
            point2.x = point1.x + myLen;
            offset = (dx < 0) ? dy : -dy;
            point2.y = point1.y;
        } else {
            myLen = 2 * sim.gridSize * Integer.signum(dy) * ((Math.abs(dy) + 2 * sim.gridSize - 1) / (2 * sim.gridSize));
            if (dy != 0) {
